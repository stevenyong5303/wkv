		...
		if(isset($_POST['ACT'])){
			switch($_POST['ACT']){
				...
				case 'lgn_chk':
					...
					if($result = $mysqli->query($sql)){
						if($result->num_rows==1){
							...
							if($row['user_level'] == '8' || $row['user_level'] == '9'){
								$sql = ' SELECT user_id, nc_name, clocked_in FROM 14RuVPJV6GmH_app_usr ' .
									   ' WHERE user_level > 1; ';
				...
				case 'evd_udt':
					...
					$mixer = ($DATA['mixer'] == '-' ? 'NULL' : addQuotes($DATA['mixer']));
					$wireless_mic = ($DATA['wmic'] == '-' ? 'NULL' : addQuotes($DATA['wmic']));
					$speaker = ($DATA['spkr'] == '-' ? 'NULL' : addQuotes($DATA['spkr']));
					...
					$bride_groom = ($DATA['bng'] == '-' ? 'NULL' : addQuotes($DATA['bng']));
